    ...
    <div className="space-y-4 m-3 rounded  ">
      <div className="bg-gray-100 flex gap-9 items-center p-3 rounded ">
        ...
        <img
          src={product.image}
          alt={product.title}
          className=" md:h-60  md:w-60  h-25 w-25   rounded  cursor-pointer"
          onClick={() => navigate(`/products/${product.id}`)}
        />
        <div className="space-y-2">
          {/* Product Details */}
          <h1 className="font-bold  md:text-xl text-lg md:line-clamp-3 line-clamp-2 hover:text-red-400  md:w-full ">
            {" "}
            {product.title}{" "}
          </h1>
          {/* Product price  */}
